...
var sonPuanHesapla=function(gelenMekan){
    ...
    if(gelenMekan.yorumlar && gelenMekan.yorumlar.length>0){
        ...
        ortalamaPuan=parseInt(toplamPuan/yorumSayisi,10);//10 10luk tabanda işlem yapma int değerinde ortpuan var
...
var ortalamaPuanGuncelle=function(mekanid){
    ...
    });//exec fonksiyonu ile sorgu çalışır
}
var yorumOlustur=function(req,res,gelenMekan) {
    if(!gelenMekan) {
        ...
        gelenMekan.yorumlar.push({
            ...
            puan: req.body.puan,
            yorumMetini: req.body.yorumMetini,
            tarih: Date.now()
        });
...
const yorumEkle = (req,res) => {
    ...
    if(mekanid) {
        ...
        res.status(201).json({mesaj: "Mekan bulunamadı."});
    }
